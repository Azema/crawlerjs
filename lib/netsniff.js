...
function getPage(url, done) {
  ...
  page.open(page.address, {encoding: 'utf8'}, function (status) {
    if (status !== 'success') {
      ...
      var time;
      var trigPaginator = function(done) {
        var retour = page.evaluate(function() {
          var paginator = jQuery('a[ng-click^="paginat"]');
          if (paginator.length === 1) {
            paginator.trigger('click');
            console.log('trigger');
            return paginator.is(':visible');
          } else {
            return null;
          }
        });
        if (retour === true) {
          ...
          time = setTimeout(trigPaginator, 300, done);
        } else {
          done();
        }
      };
      trigPaginator(function() {
        page.hasJQuery = page.evaluate(function () {
          if (typeof jQuery === 'function') {
            var findLinks = function() {
              var links = [], $links = jQuery('a'),
